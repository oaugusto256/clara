...
function parseCsv(raw) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        for (const [idx, key] of Object.entries(headerMap)) {
            ...
            const raw = (_a = cols[index]) !== null && _a !== void 0 ? _a : '';
            if (key === 'amount')
                obj.amount = parseAmountString(raw);
            else if (key === 'date')
                obj.date = parseDateString(raw);
            else
                obj[key] = raw;
        }
        ...
        if (!obj.currency)
            obj.currency = 'BRL';
        try {
            const parsed = schemas_1.NormalizedTransactionInputSchema.parse(obj);
            result.ok.push(parsed);
        }
        catch (err) {
            result.errors.push({ line: i + 1, reason: (_b = err === null || err === void 0 ? void 0 : err.message) !== null && _b !== void 0 ? _b : 'validation failed' });
        }
    }
    return result;
}
